      ...
      <div v-if="data">
        <bm-marker  v-for="(item, index) in data" :key="index" @click="infoWindowOpen(item)" :clicking="true" :position="item.markerPoint" :dragging="false" >
          <bm-info-window :show="item.infoWindowShowFlag" @close="infoWindowClose(item)" @open="infoWindowOpen(item)">
            <div class="info">
              <div class="itemList">
                <div><span>库房名称：</span>{{item.housename}}</div>
              ...
              <div class="link">
                <router-link :to="{path: `/monitoring/storehouse`, query: {id: item.companyid}}">库房实时监控</router-link>
              </div>
            </div>
          </bm-info-window>
        </bm-marker>
...
export default class extends Vue {
  @Prop({ default: [] }) private data?: any
   private show = ''
   public infoWindowClose(item: any) {
     this.show = ''
   }
